...
Pressure is fail-open: provider forwarding, request/execution skeleton transitions, and compact Usage Log statistics continue. A Usage Log can conservatively take the charged total past the hard watermark; AxonHub keeps pressure set so cleanup runs and later optional primary-database request bodies, request headers, response bodies, and response chunks are denied until capacity recovers. Usage accounting or managed-group marking failures are reported as degraded managed-observability signals without discarding an already saved Usage Log; a database error that prevents the Usage Log insert is still returned to the caller. Structured `managed_observability_*` signals, `managed_observability_failures_total{component,reason}`, and the singleton `managed_observability_states.last_error` component value describe admission, owner-lock, unlock, accounting, and cleanup degradation. Public health reports this component state but remains healthy; health/liveness must not be wired to fail solely because capacity is under pressure.
