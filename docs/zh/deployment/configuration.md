...
SSE 心跳目前应用于 OpenAI 兼容和 Anthropic 兼容的流式 API，暂不应用于 Gemini 流式 API。渠道可通过 `settings.sseKeepAlive.enabled` 和 `settings.sseKeepAlive.intervalSeconds` 分别覆盖全局值；省略的字段继承服务器配置。心跳仅用于传输层保活，不会形成响应分块、用量或重试。
