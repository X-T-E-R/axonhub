...
func TestDatabaseTimeScan_DriverShapes(t *testing.T) {
	want := time.Date(2026, 8, 22, 3, 4, 5, 123456789, time.UTC)
	...
	tests := []struct {
		...
		{name: "postgres pgx time", value: want, valid: true},
		{name: "postgres RFC3339 text", value: want.Format(time.RFC3339Nano), valid: true},
		{name: "sqlite bytes", value: []byte(want.Format("2006-01-02 15:04:05.999999999 -0700 MST")), valid: true},
		{name: "aggregate NULL", value: nil, valid: false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var got databaseTime
			require.NoError(t, got.Scan(tt.value))
			require.Equal(t, tt.valid, got.Valid)
			if tt.valid {
				require.True(t, got.Time.Equal(want), "got %v, want %v", got.Time, want)
			}
		})
	}

	var invalid databaseTime
