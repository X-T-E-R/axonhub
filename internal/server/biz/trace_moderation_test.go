...
func TestExtractSpansFromModerationResponseBody(t *testing.T) {
	...
	require.Contains(t, spans[0].Value.Text.Text, "violence=0.9000")
	require.Contains(t, spans[0].Value.Text.Text, "omni-moderation-latest")
}

func TestModerationTraceSummaryBoundsLargeBodies(t *testing.T) {
	...
	spans := extractSpansFromModerationRequestBody(body, "request-large")
	require.Len(t, spans, 1)
	summary := spans[0].Value.UserQuery.Text
	require.Less(t, len(summary), moderationTraceSummaryLimit+256)
	require.Contains(t, summary, "image_url(length=")
	require.Contains(t, summary, "sha256=")
	require.NotContains(t, summary, strings.Repeat("A", 1024))
}
