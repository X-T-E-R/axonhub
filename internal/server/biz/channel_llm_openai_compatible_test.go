...
func TestAxonHubChannel_BuildChannelWithOutbounds_UsesEndpointSpecificTransformers(t *testing.T) {
	...
	require.NotNil(t, built)
	require.NotNil(t, built.Outbound)
	require.Len(t, built.Outbounds, 17)
	...
	for _, apiFormat := range []llm.APIFormat{
		...
		t.Run(apiFormat.String(), func(t *testing.T) {
			audioOutbound, err := BuildOutboundByAPIFormat(built, apiFormat.String())
			require.NoError(t, err)
			require.NotNil(t, audioOutbound)
			_, ok := audioOutbound.(*openai.OutboundTransformer)
			require.True(t, ok, "%s endpoint should use openai outbound", apiFormat)
		})
	}
