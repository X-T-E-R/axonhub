...
test('AxonHub exposes every routed OpenAI audio format', () => {
  const providersConfig = read('features/channels/data/config_providers.ts');
  const formatsMatch = providersConfig.match(
    /const AXONHUB_PROVIDER_API_FORMATS:\s*ApiFormat\[\]\s*=\s*\[([\s\S]*?)\];/,
  );

  assert.ok(formatsMatch, 'AXONHUB_PROVIDER_API_FORMATS should be defined');
  for (const apiFormat of [
    'openai/audio_speech',
    'openai/audio_transcriptions',
    'openai/audio_translations',
  ]) {
    assert.match(formatsMatch[1], new RegExp(`['"]${apiFormat}['"]`));
  }
});
