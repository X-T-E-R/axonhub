...
func (t *ModerationInboundTransformer) TransformRequest(
	...
	if modReq.Stream != nil && *modReq.Stream {
		return nil, fmt.Errorf("%w: moderations do not support streaming", transformer.ErrInvalidRequest)
	}

	if err := normalizeAndValidateModerationInput(&modReq.Input); err != nil {
		return nil, err
	}
...
func normalizeAndValidateModerationInput(input *llm.ModerationInput) error {
	if input == nil {
		return fmt.Errorf("%w: input is required", transformer.ErrInvalidRequest)
	}
	...
	if len(input.Parts) > 0 {
		for i := range input.Parts {
			...
			case "image_url":
				...
				part.ImageURL.URL = strings.TrimSpace(part.ImageURL.URL)
	...
	if input.StringArray != nil {
		...
		for i := range input.StringArray {
			input.StringArray[i] = strings.TrimSpace(input.StringArray[i])
	...
	input.String = strings.TrimSpace(input.String)
	if input.String == "" {
