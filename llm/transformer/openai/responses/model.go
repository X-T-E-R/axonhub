...
type Item struct {
	// Agent envelopes are rendered for Chat but replayed as received for native
	// Responses, including opaque metadata unknown to this gateway.
	RawAgentMessage json.RawMessage `json:"-"`
	// ArgumentsPresent distinguishes an omitted terminal snapshot field from an
	// explicitly empty one. It is aggregation bookkeeping, not part of the
	...
	// Sender and recipient for Codex inter-agent messages.
...
func (item *Item) UnmarshalJSON(data []byte) error {
	...
	*item = Item(raw.itemAlias)
	if item.Type == "agent_message" {
		item.RawAgentMessage = append(json.RawMessage(nil), data...)
	}
	item.ArgumentsPresent = len(raw.Arguments) > 0 && !bytes.Equal(raw.Arguments, []byte("null"))
...
func (item Item) MarshalJSON() ([]byte, error) {
	...
	if item.Type == "agent_message" && len(item.RawAgentMessage) > 0 {
		return item.RawAgentMessage, nil
	}
