...
func TestModerationInboundTransformer_TransformRequest(t *testing.T) {
	...
	t.Run("normalizes multimodal part values before forwarding", func(t *testing.T) {
		llmReq, err := transformer.TransformRequest(context.Background(), &httpclient.Request{
			Body:    []byte(`{"input":[{"type":"  text ","text":"  classify this  "},{"type":" image_url ","image_url":{"url":" https://example.com/a.png "}}]}`),
			Headers: http.Header{"Content-Type": []string{"application/json"}},
		})
		require.NoError(t, err)
		require.Equal(t, "text", llmReq.Moderation.Input.Parts[0].Type)
		require.Equal(t, "classify this", llmReq.Moderation.Input.Parts[0].Text)
		require.Equal(t, "image_url", llmReq.Moderation.Input.Parts[1].Type)
		require.Equal(t, "https://example.com/a.png", llmReq.Moderation.Input.Parts[1].ImageURL.URL)
	})

	t.Run("stream true is explicitly rejected", func(t *testing.T) {
		_, err := transformer.TransformRequest(context.Background(), &httpclient.Request{
			Body:    []byte(`{"input":"hello","stream":true}`),
			Headers: http.Header{"Content-Type": []string{"application/json"}},
		})
		require.Error(t, err)
		require.Contains(t, err.Error(), "moderations do not support streaming")
	})

	t.Run("stream false remains non-streaming", func(t *testing.T) {
		llmReq, err := transformer.TransformRequest(context.Background(), &httpclient.Request{
